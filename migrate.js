...
features.forEach(feature => {
  ...
  // Copy components
  const compDir = path.join(__dirname, 'frontend', 'components', feature);
  const targetCompDir = path.join(srcAppDir, 'components');
  if (fs.existsSync(compDir)) {
    copyFolderSync(compDir, targetCompDir);
  }
  
  // Copy common UI components if needed
  const uiCompDir = path.join(__dirname, 'frontend', 'components', 'ui');
  const targetUiCompDir = path.join(srcAppDir, 'components', 'ui');
  if (fs.existsSync(uiCompDir)) {
    copyFolderSync(uiCompDir, targetUiCompDir);
  }
